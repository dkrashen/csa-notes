...
\begin{defn} \index{simple module}
...
\begin{defn} \index{Jacobson Radical}
...
It therefore makes sense to define the Jacobson radical \index{Jacobson
Radical} of $R$, to be $J(R)
= J_r(R) = J_\ell(R)$.

\begin{defn} \index{Semiprimitive}
...
modules:
\begin{thm}[Schur's Lemma] \index{Schur's Lemma}
...
\begin{thm}[Wedderburn-Artin] \index{Wedderburn-Artin Theorem}
...
\subsection{Definition and universal property} \index{Tensor products of
algebras}
...
\subsection{Description via structure constants}
...
Recall that if $R$ is a ring, $L \subset R$ any subset, the centralizer
\index{Centralizer} of
...
\begin{thm}[Double Centralizer, Version 1] \label{dc1} \index{Double
Commutator Theorem!Version 1}
